...
class Pong:
    ...
    def __init__(self):
        ...
        self.ai_paddles.append(ComputerPaddle(  # The left paddle
            ...
            pygame.K_s,
            0,
            int(self.HEIGHT / 2 - self.PADDLE_HEIGHT / 2),
            self.PADDLE_WIDTH,
            self.PADDLE_HEIGHT
        ))
        ...
        self.central_line = pygame.Rect(int(self.WIDTH/2), 0, 1, self.HEIGHT)

    def check_ball_hits_wall(self):
        for ball in self.balls:
            if ball.x > self.WIDTH or ball.x < 0:
                ball.x = int(self.WIDTH / 2)
                ball.angle = random.randint(-10, 10)
                ball.velocity *= random.choice([-1, 1])
